...
{
    return __bdk_if_ops_xaui.if_num_interfaces();
}
...
static int if_num_ports(int interface)
{
    ...
    int ports = __bdk_if_ops_xaui.if_num_ports(-interface-1);
    if (ports == 0)
        return ports;
...
static int if_init(bdk_if_handle_t handle)
{
    ...
    BDK_CSR_MODIFY(pip_prt_cfg, BDK_PIP_PRT_CFGX(handle->pknd),
    ...
    BDK_CSR_MODIFY(gmx_rx_udd_skp, BDK_GMXX_RXX_UDD_SKP_2(gmx_block, 0),
        gmx_rx_udd_skp.s.len = header_size;
        gmx_rx_udd_skp.s.fcssel = 0);

    /* Disable GMX preamble checking */
    BDK_CSR_MODIFY(gmx_rx_frm_ctl, BDK_GMXX_RXX_FRM_CTL_2(gmx_block, 0),
        gmx_rx_frm_ctl.s.pre_chk = 0);

    /* Setup GMX TX to pad properly min sized packets */
    BDK_CSR_MODIFY(gmx_tx_min_pkt, BDK_GMXX_TXX_MIN_PKT_2(gmx_block, 0),
        gmx_tx_min_pkt.s.min_size = 59 + header_size);

    /* Setup GMX TX to not add a preamble */
    BDK_CSR_MODIFY(gmx_tx_append, BDK_GMXX_TXX_APPEND_2(gmx_block, 0),
        gmx_tx_append.s.preamble = 0);

    /* Reduce the inter frame gap to 8 bytes */
    BDK_CSR_MODIFY(gmx_tx_ifg, BDK_GMXX_TX_IFG(gmx_block),
        gmx_tx_ifg.s.ifg1 = 4;
        gmx_tx_ifg.s.ifg2 = 4);

    /* Disable GMX backpressure */
    BDK_CSR_MODIFY(gmx_tx_ovr_bp, BDK_GMXX_TX_OVR_BP(gmx_block),
    ...
    if (header_size == 16)
    {
        /* Enable HiGig2 support and forwarding of virtual port backpressure
            to PKO */
        BDK_CSR_MODIFY(gmx_hg2_control, BDK_GMXX_HG2_CONTROL(gmx_block),
    ...
    /* Enable HiGig */
    BDK_CSR_MODIFY(gmx_tx_xaui_ctl, BDK_GMXX_TX_XAUI_CTL(gmx_block),
