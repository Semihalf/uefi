...
class Connection:
    def __init__(self, console="/dev/ttyS0", mcu="/dev/ttyS0", mcu2="/dev/ttyS0", logname="test.log"):
        ...
        if os.system("stty --file=" + mcu + " " + str(baudrate)):
            raise Exception("stty failed")
        if os.system("stty --file=" + mcu2 + " " + str(baudrate)):
            raise Exception("stty failed")
        # Open the serial ports
        fd_console = os.open(console, os.O_RDWR | os.O_SYNC | os.O_NDELAY)
        fd_mcu = os.open(mcu, os.O_RDWR | os.O_SYNC | os.O_NDELAY)
        fd_mcu2 = os.open(mcu2, os.O_RDWR | os.O_SYNC | os.O_NDELAY)

        self.logging = Log(logname)
        self.fd_console = fd_console
        self.fd_mcu = fd_mcu
        self.fd_mcu2 = fd_mcu2
    ...
    def close(self):
        self.logging.close()
        os.close(self.fd_console)
        os.close(self.fd_mcu)
        os.close(self.fd_mcu2)
    ...
    def powerCycle(self):
        ...
        os.write(self.fd_mcu, "\rP -f -r\r")
        os.write(self.fd_mcu2, "\rP -f -r\r")
        #os.fsync(self.fd_console);
