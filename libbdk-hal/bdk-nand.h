...
#define BDK_NAND_MAX_PAGE_AND_OOB_SIZE      (4096 + 256)


/* Block size for boot ECC */
#define BDK_NAND_BOOT_ECC_BLOCK_SIZE    (256)
/* ECC bytes for each block */
#define BDK_NAND_BOOT_ECC_ECC_SIZE      (8)
...
typedef enum
{
    BDK_NAND_INITIALIZE_FLAGS_16BIT = 1<<0,
    BDK_NAND_INITIALIZE_FLAGS_DONT_PROBE = 1<<1,
    BDK_NAND_INITIALIZE_FLAGS_DEBUG = 1<<15,
} bdk_nand_initialize_flags_t;
...
{
    BDK_NAND_SUCCESS = 0,
    BDK_NAND_NO_MEMORY = -1,
    BDK_NAND_BUSY = -2,
    BDK_NAND_INVALID_PARAM = -3,
    BDK_NAND_TIMEOUT = -4,
    BDK_NAND_ERROR = -5,
} bdk_nand_status_t;
...
typedef struct
{
    ...
    uint64_t reserved_4_7       : 4;
    uint64_t eleven             : 4;
} bdk_nand_cmd_wait_status_ale_t;
...
{
    uint64_t u64[2];
    bdk_nand_cmd_nop_t             nop;
    bdk_nand_cmd_set_tm_par_t      set_tm_par;
    bdk_nand_cmd_wait_t            wait;
    bdk_nand_cmd_chip_en_t         chip_en;
    bdk_nand_cmd_chip_dis_t        chip_dis;
    bdk_nand_cmd_cle_t             cle;
    bdk_nand_cmd_ale_t             ale;
    bdk_nand_cmd_rd_t              rd;
    bdk_nand_cmd_rd_edo_t          rd_edo;
    bdk_nand_cmd_wr_t              wr;
    bdk_nand_cmd_wait_status_t     wait_status;
    bdk_nand_cmd_wait_status_ale_t wait_status_ale;
    bdk_nand_cmd_bus_acq_t         bus_acq;
    bdk_nand_cmd_bus_rel_t         bus_rel;
    struct
    {
        uint64_t reserved_64_127: 64;
        uint64_t reserved_4_63  : 60;
 ...
 */
extern bdk_nand_status_t bdk_nand_initialize(bdk_nand_initialize_flags_t flags, int active_chips);



/**
 * This function may be called before bdk_nand_initialize to set default values that will be used
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_set_defaults(int page_size, int oob_size, int pages_per_block, int blocks, int onfi_timing_mode);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_shutdown(void);
 ...
 */
extern int bdk_nand_get_active_chips(void);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_set_timing(int chip, int tim_mult, int tim_par[7], int clen[4], int alen[4], int rdn[4], int wrn[2]);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_submit(bdk_nand_cmd_t cmd);
 ...
 */
extern int bdk_nand_page_read(int chip, uint64_t nand_address, uint64_t buffer_address, int buffer_length);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_page_write(int chip, uint64_t nand_address, uint64_t buffer_address);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_block_erase(int chip, uint64_t nand_address);
 ...
 * @return Bytes read on success, a negative bdk_nand_status_t error code on failure
 */
extern int bdk_nand_read_id(int chip, uint64_t nand_address, uint64_t buffer_address, int buffer_length);
 ...
 * @return Bytes read on success, a negative bdk_nand_status_t error code on failure
 */
extern int bdk_nand_read_param_page(int chip, uint64_t buffer_address, int buffer_length);
 ...
 */
extern int bdk_nand_get_status(int chip);
 ...
 */
extern int bdk_nand_get_page_size(int chip);
 ...
 */
extern int bdk_nand_get_oob_size(int chip);
 ...
 * @param chip   Chip select for NAND flash
 *
 * @return Numboer of pages in each block or a negative bdk_nand_status_t error code on failure
 */
extern int bdk_nand_get_pages_per_block(int chip);
 ...
 */
extern int bdk_nand_get_blocks(int chip);
 ...
 * @return Zero on success, a negative bdk_nand_status_t error code on failure
 */
extern bdk_nand_status_t bdk_nand_reset(int chip);
 ...
 */
extern void bdk_nand_compute_boot_ecc(unsigned char *block, unsigned char *eccp);
