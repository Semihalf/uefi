...
const dram_config_t *libdram_config_load(bdk_node_t node)
{
    ...
    for (int lmc = 0; lmc < MAX_LMCS; lmc++)
    {
        ...
        custom->ddr_rtt_nom_auto                = bdk_config_get_int(BDK_CONFIG_DDR_CUSTOM_DDR_RTT_NOM_AUTO,    lmc, node);
        custom->ddr_rodt_ctl_auto               = bdk_config_get_int(BDK_CONFIG_DDR_CUSTOM_DDR_RODT_CTL_AUTO,   lmc, node);
        ...
        custom->rlevel_average_loops            = bdk_config_get_int(BDK_CONFIG_DDR_CUSTOM_RLEVEL_AVERAGE_LOOPS,lmc, node);
