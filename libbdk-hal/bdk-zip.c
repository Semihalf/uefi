...
static bdk_cmd_queue_state_t zip_queue;
...
int bdk_zip_initialize(void)
{
    ...
    result = bdk_cmd_queue_initialize(&zip_queue, BDK_FPA_OUTPUT_BUFFER_POOL,
    ...
    zip_cmd_buf.s.ptr =  bdk_ptr_to_phys(bdk_cmd_queue_buffer(&zip_queue))>>7;
...
int bdk_zip_shutdown(void)
{
    ...
    bdk_cmd_queue_shutdown(&zip_queue);
    return 0;
}
...
{
    bdk_cmd_queue_result_t result = bdk_cmd_queue_write(&zip_queue, 1, 8, command->u64);
    if (result == BDK_CMD_QUEUE_SUCCESS)
