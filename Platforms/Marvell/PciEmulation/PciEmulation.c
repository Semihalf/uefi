  ...
  )
{
  ...
  for (i = 0; i < DevCount; i++) {
    while ((--Walker) >= PcdString) {
      if (*Walker == L';') {
        // Cut off parsed chunk from PCD string by replacing ';' with CRLF
        *Walker = '\0';
        Tmp = ShellHexStrToUintn ((Walker + 1));
        if ((UINTN)(-1) == Tmp) {
          return EFI_INVALID_PARAMETER;
        }
        // We parse from the end to the beginning
        // so fulfill table in the same manner
        ValueTable[DevCount - (i + 1)] = Tmp;
        Walker--;
        break;
      }
      if (Walker == PcdString) {
        ...
        // We should fill table for last device here. If not, return error
        if (i != (DevCount - 1)
          DEBUG((DEBUG_ERROR, "PciEmulation: Please set PCD value for every"
            "device\n"));
          return EFI_INVALID_PARAMETER;
        }
        // We parse from the end to the beginning
