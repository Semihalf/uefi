-- Configure the BDK for a evb7000 board.
print("Configuring for the EVB7000_sff")
local bit64 = require("bit64")

local set_config = octeon.c.bdk_config_set
octeon.c.bdk_gpio_initialize(0, 9, 1, 0);

-- RGMII PHY address is 4, on SMI0
set_config(octeon.CONFIG_PHY_MGMT_PORT0, 4)

-- (Q)SGMII PHY address for interface 0 are 0-3, on SMI0
set_config(octeon.CONFIG_PHY_IF0_PORT0, 0)
set_config(octeon.CONFIG_PHY_IF0_PORT1, 1)
set_config(octeon.CONFIG_PHY_IF0_PORT2, 2)
set_config(octeon.CONFIG_PHY_IF0_PORT3, 3)

-- (Q)SGMII PHY address for interface 1 are 4-7, on SMI1
set_config(octeon.CONFIG_PHY_IF1_PORT0, 256 + 4)
set_config(octeon.CONFIG_PHY_IF1_PORT1, 256 + 5)
set_config(octeon.CONFIG_PHY_IF1_PORT2, 256 + 6)
set_config(octeon.CONFIG_PHY_IF1_PORT3, 256 + 7)

-- RXAUI PHY address is 18, hex 0x12, on SMI1
-- set_config(octeon.CONFIG_PHY_IF0_PORT0, 256 + 18)

-- TWSI info
-- DDR0 converter: Address 0x31
-- Core converter: Address 0x40
-- DDR0 Rom: Address 0x50
-- EEPROM: Address 0x56
-- QLM Rom: Address 0x57
-- DDR clock control: Address 0x58
-- PNR clock control: Address 0x5b
-- PLL clock control: Address 0x5a


--
-- Setup Vitesse PHYs
-- This function sets up one port in a Vitesse VSC8574 for
-- either SGMII or QSGMII
--
local function setup_vitesse_phy(mdio_bus, phy_addr, qsgmii)
    local node = 0
    -- Select "G" registers
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x10)
    -- Reg 19G, bit 15:14
    -- 0 = SGMII
    -- 1 = QSGMII
    local reg19 = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 19)
    local reg18
    if qsgmii then
        -- QSGMII
        reg19 = bit64.binsert(reg19, 1, 14, 15)
        reg18 = 0x80e0
    else
        -- SGMII
        reg19 = bit64.binsert(reg19, 0, 14, 15)
        reg18 = 0x80f0
    end
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 19, reg19)
    -- Write 18G, change all 4 ports
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 18, reg18)
    -- Select main registers
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0)
    -- Reg23, 10:8 Select copper
    local reg23 = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 23)
    reg23 = bit64.binsert(23, 0, 8, 10)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 23, reg23)
    -- Reg0, Reset
    local reg0 = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 0)
    reg0 = bit64.binsert(reg0, 1, 15, 15)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 0, reg0)
    -- Reg 16E3, bit 7 auto negotiation
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 3)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 16, 0x80)
    -- Select main registers
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0)
    -- Near end loopback (Octeon side)
    if false then
        reg0 = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 0)
        reg0 = bit64.binsert(reg0, 1, 14, 14)
        octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 0, reg0)
    end

    -- Far end loopback (External side)
    if false then
        reg23 = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 23)
        reg23 = bit64.binsert(reg23, 1, 3, 3)
        octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 23, reg23)
    end

    -- Dump registers
    if false then
        printf("\nVitesse PHY register dump, PHY address %d, mode %s\n",
               phy_addr, qsgmii and "QSGMII" or "SGMII")
        local phy_addr = 4
        for reg_set = 0,0x10,0x10 do
            printf("\nDump registers with reg[31]=0x%x\n", reg_set)
            octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, reg_set)
            for reg=0,31 do
                printf("reg[%02d]=0x%x\n", reg, octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, reg))
            end
        end
    end
end

local patch_arr = {
        }

local function wr_masked(node, mdio_bus, phy_addr, reg, value, mask)
    local nmask = bit64.bnot(mask)
    local old = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, reg)
    local vmask = bit64.band(value, mask)
    local newv = bit64.band(old, nmask)
    newv = bit64.bor(newv, vmask)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, reg, value)
end

local function vitesse_program(mdio_bus, phy_addr)
    local node = 0
    printf("Programming Vitesse PHY at address %d\n", phy_addr)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0010)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 18, 0x800f)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0010)

    local reg18g = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 18)
    local timeout = 10
    while bit64.btest(reg18g, 15) and (timeout > 0) do
        octeon.c.bdk_wait_usec(100000)
        reg18g = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 18)
        timeout = timeout - 1
    end

    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0000)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0010)
    wr_masked(node, mdio_bus, phy_addr, 12, 0x0000, 0x0800)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr,  9, 0x005b)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 10, 0x005b)
    wr_masked(node, mdio_bus, phy_addr, 12, 0x0800, 0x0800)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 18, 0x800f)
    wr_masked(node, mdio_bus, phy_addr, 0, 0x0000, 0x8000)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 18, 0x0000)
    wr_masked(node, mdio_bus, phy_addr, 12, 0x0000, 0x0800)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x10)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 0, 0x7009)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 12, 0x5002)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 11, 0x0000)

    for _,data in ipairs(patch_arr) do
        local d = bit64.bor(0x5000, data)
        octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 12, d)
    end
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 12, 0x0000)

    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr,  3, 0x3eb7)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr,  4, 0x4012)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 12, 0x0100)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr,  0, 0x4018)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr,  0, 0xc018)

    -- below verifies CRC is correct in 8051 RAM.  CRC is 16-bit.
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0001)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 25, 0x4000)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 26, #patch_arr + 1)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0010)
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 18, 0x8008)

    reg18g = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 18)
    timeout = 10
    while bit64.btest(reg18g, 15) and (timeout > 0) do
        octeon.c.bdk_wait_usec(100000)
        reg18g = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 18)
        timeout = timeout - 1
    end

    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0001)

    local crc_calculated = octeon.c.bdk_mdio_read(node, mdio_bus, phy_addr, 25)
    if crc_calculated ~= 0xB7C2 then
        printf("8051 crc_calculated = 0x%x, expected_crc = 0x%x\n", crc_calculated, 0xB7C2)
    end
    octeon.c.bdk_mdio_write(node, mdio_bus, phy_addr, 31, 0x0000)
end

function setup_dlm0_hook(mode, speed, flags)
    local qsgmii_lane0 = ((mode == octeon.QLM_MODE_QSGMII_SGMII) or
                          (mode == octeon.QLM_MODE_QSGMII_QSGMII) or
                          (mode == octeon.QLM_MODE_QSGMII_DISABLED))
    local qsgmii_lane1 = ((mode == octeon.QLM_MODE_SGMII_QSGMII) or
                          (mode == octeon.QLM_MODE_QSGMII_QSGMII) or
                          (mode == octeon.QLM_MODE_DISABLED_QSGMII))
    if qsgmii_lane0 then
        vitesse_program(0, 0)
    end
    for phy_addr=0,3 do
        setup_vitesse_phy(0, phy_addr, qsgmii_lane0)
    end

    if qsgmii_lane1 then
        vitesse_program(1, 4)
    end
    for phy_addr=4,7 do
        setup_vitesse_phy(1, phy_addr, qsgmii_lane1)
    end
end

