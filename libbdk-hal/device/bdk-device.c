...
static void populate_device(bdk_device_t *device)
{
    ...
    while (cap_loc)
    {
        ...
        {
            BDK_TRACE(DEVICE_SCAN, "%s:      Enhanced Allocation\n", device->name);
            // FIXME
        }
        else
        {
...
uint64_t bdk_bar_read(const bdk_device_t *device, int bar, int size, uint64_t offset)
{
    ...
    switch (size)
    {
        ...
        case 2:
            return bdk_read64_uint16(address);
        case 4:
            return bdk_read64_uint32(address);
        case 8:
...
void bdk_bar_write(const bdk_device_t *device, int bar, int size, uint64_t offset, uint64_t value)
{
    ...
    switch (size)
    {
        case 1:
            ...
            return;
        case 2:
            bdk_write64_uint16(address, value);
            return;
        case 4:
            bdk_write64_uint32(address, value);
            return;
        case 8:
