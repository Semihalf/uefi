...
static inline const char* __mask_to_str(uint64_t mask)
{
    ...
    uint64_t non_1k_mask = mask + BDK_TLB_PAGEMASK_4K;

    switch (non_1k_mask) {
    case BDK_TLB_PAGEMASK_4K:     return "4kb";
    case BDK_TLB_PAGEMASK_16K:    return "16kb";
    case BDK_TLB_PAGEMASK_64K:    return "64kb";
    case BDK_TLB_PAGEMASK_256K:   return "256kb";
    case BDK_TLB_PAGEMASK_1M:     return "1Mb";
    case BDK_TLB_PAGEMASK_4M:     return "4Mb";
    case BDK_TLB_PAGEMASK_16M:    return "16Mb";
    case BDK_TLB_PAGEMASK_64M:    return "64Mb";
    case BDK_TLB_PAGEMASK_256M:   return "256Mb";
    }
...
static inline int __tlb_read_index(uint32_t tlbi){
    ...
    if (tlbi >= bdk_tlb_size_limit()) {
        return -1;
    }

    BDK_MT_TLB_INDEX(tlbi);
    __tlb_read();
...
static inline int __tlb_entry_is_free(uint32_t tlbi) {
    ...
    if (tlbi < bdk_tlb_size_limit()) {
    	...
    	BDK_MF_ENTRY_LO_0(lo0);
    	BDK_MF_ENTRY_LO_1(lo1);
...
static inline void __tlb_dump_index(uint32_t tlbi)
{
    if (tlbi < bdk_tlb_size_limit()) {
        ...
        if (__tlb_entry_is_free(tlbi)) {
            ...
            BDK_MF_ENTRY_HIGH(hi);
            BDK_MF_ENTRY_LO_0(lo0);
            BDK_MF_ENTRY_LO_1(lo1);
            BDK_MF_PAGEMASK(pgmask);
                              ...
                              uint64_t lo1, uint64_t pagemask)
{
    ...
    if (index < bdk_tlb_size_limit() && __tlb_entry_is_free(index) ) {
...
int bdk_tlb_allocate_runtime_entry(void)
{
    ...
    for (i = __tlb_wired_index(); i< bdk_tlb_size_limit(); i++) {
...
void bdk_tlb_free_runtime_entry(uint32_t tlbi)
{
    ...
    if ((tlbi < bdk_tlb_size_limit()) && (tlbi >= __tlb_wired_index())) {
 ...
 * 	     0:  fixed entry added
 */
int bdk_tlb_add_fixed_entry( uint64_t vaddr, uint64_t paddr, uint64_t size, uint64_t tlb_flags) {
    ...
    /* Check to make sure if the index is free to use */
    if (index < bdk_tlb_size_limit() && __tlb_entry_is_free(index) ) {
	bdk_tlb_write_entry(index, vaddr, paddr, size, tlb_flags);
...
void bdk_tlb_dump_shared_mapping(void) {
    ...
    for ( tlbi = __tlb_wired_index(); tlbi<bdk_tlb_size_limit(); tlbi++ ) {
...
void bdk_tlb_dump_all(void) {
    ...
    for (tlbi = 0; tlbi<= bdk_tlb_size_limit(); tlbi++ ) {
        __tlb_dump_index(tlbi);
    }
}
