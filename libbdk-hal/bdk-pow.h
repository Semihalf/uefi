...
static inline bdk_pow_tag_req_t bdk_pow_get_current_tag(void)
{
    ...
    load_addr.sstatus.did = BDK_OCT_DID_TAG_TAG1;
    load_addr.sstatus.coreid = bdk_get_core_num();
    load_addr.sstatus.get_cur = 1;
    load_resp.u64 = bdk_read64_uint64(load_addr.u64);
    result.u64 = 0;
...
static inline bdk_wqe_t *bdk_pow_get_current_wqp(void)
{
    ...
    load_resp.u64 = bdk_read64_uint64(load_addr.u64);
    return (bdk_wqe_t*)bdk_phys_to_ptr(load_resp.s_sstatus4.wqp);
}
...
static inline void __bdk_pow_warn_if_pending_switch(const char *function)
{
    ...
    bdk_warn_if(!switch_complete, "%s called with tag switch in progress\n", function);
}
...
static inline bdk_wqe_t * bdk_pow_work_request_sync_nocheck(bdk_pow_wait_t wait)
{
    ...
    ptr.swork.is_io = 1;
    ptr.swork.did = BDK_OCT_DID_TAG_SWTAG;
    ptr.swork.wait = wait;
    ...
    if (result.s_work.no_work)
        ...
        return (bdk_wqe_t*)bdk_phys_to_ptr(result.s_work.addr);
}
...
static inline bdk_pow_tag_type_t bdk_pow_work_request_null_rd(void)
{
    ...
    ptr.snull_rd.did = BDK_OCT_DID_TAG_NULL_RD;
...
static inline void bdk_pow_work_request_async_nocheck(int scr_addr, bdk_pow_wait_t wait)
{
    ...
    data.s.len = 1;
    data.s.did = BDK_OCT_DID_TAG_SWTAG;
    data.s.wait = wait;
...
static inline void bdk_pow_tag_sw_null_nocheck(void)
{
    ...
    ptr.sio.mem_region = BDK_IO_SEG;
    ptr.sio.is_io = 1;
    ptr.sio.did = BDK_OCT_DID_TAG_TAG1;
...
static inline void bdk_pow_tag_sw_desched_nocheck(uint32_t tag, bdk_pow_tag_type_t tag_type, uint64_t group, uint64_t no_sched)
{
    ...
    ptr.sio.mem_region = BDK_IO_SEG;
    ptr.sio.is_io = 1;
    ptr.sio.did = BDK_OCT_DID_TAG_TAG3;

    bdk_write64_uint64(ptr.u64, tag_req.u64); /* since TAG3 is used, this store will clear the local pending switch bit */
}
/**
 * Performs a tag switch and then an immediate deschedule. This completes
 * immediatly, so completion must not be waited for.  This function does NOT
 ...
 */
#define BDK_TAG_SW_BITS    (8)
#define BDK_TAG_SW_SHIFT   (32 - BDK_TAG_SW_BITS)

/* Below is the list of values for the top 8 bits of the tag. */
#define BDK_TAG_SW_BITS_INTERNAL  0x1  /* Tag values with top byte of this value are reserved for internal executive uses */
...
#define BDK_TAG_SUBGROUP_MASK  0xFFFF /* Mask for the value portion of the tag */
#define BDK_TAG_SUBGROUP_SHIFT 16
#define BDK_TAG_SUBGROUP_PKO  0x1
