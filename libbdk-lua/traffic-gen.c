...
static void tg_init_port(trafficgen_port_info_t *pinfo)
{
    ...
    switch (pinfo->iftype)
    {
        case BDK_IF_SGMII:
        case BDK_IF_XAUI:
            BDK_CSR_WRITE(BDK_GMXX_RXX_JABBER(__bdk_if_get_gmx_index(pinfo->priv.handle),__bdk_if_get_gmx_block(pinfo->priv.handle)), 65535);
            BDK_CSR_MODIFY(pip_frm_len_chkx, BDK_PIP_FRM_LEN_CHKX(bdk_if_get_pknd(pinfo->priv.handle)),
    ...
    BDK_CSR_MODIFY(port_cfg, BDK_PIP_PRT_CFGX(bdk_if_get_pknd(pinfo->priv.handle)),
        port_cfg.s.dyn_rs = 1);
...
static void tg_init(void)
{
    ...
    for (bdk_if_handle_t handle = bdk_if_next_port(NULL); handle!=NULL; handle = bdk_if_next_port(handle))
    {
        ...
        if (pinfo)
        {
            ...
            sprintf(pinfo->name, "eth%d", count);
            tg_init_port(pinfo);
...
int trafficgen_do_update(void)
{
    ...
    for (int i=0; tg_all_set.list[i] != NULL; i++)
    {
        ...
        bdk_pko_get_port_status(bdk_if_get_pknd(pinfo->priv.handle), 0, &pko_stats);
        ...
        bdk_pip_get_port_status(bdk_if_get_pknd(pinfo->priv.handle), 0, &pip_stats);
...
static packet_free_t fastpath_receive(trafficgen_port_info_t *pinfo, bdk_if_packet_t *packet)
{
    ...
    {
        bdk_atomic_add64((int64_t*)&pinfo->stats.rx_wqe_errors[packet->rx_error], 1);
#if 0 // FIXME
