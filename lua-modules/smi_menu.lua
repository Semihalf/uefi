...
local function smi_scan(smi_bus)
    ...
    for dev_addr = 0, 31 do
        local data = octeon.c.bdk_mdio_read(node, smi_bus, dev_addr, 0)
...
local function smi_read(smi_bus)
    ...
    local data = octeon.c.bdk_mdio_read(node, smi_bus, dev_addr, reg)
...
local function smi_write(smi_bus)
    ...
    assert(octeon.c.bdk_mdio_write(node, smi_bus, dev_addr, reg, data) ~= -1, "SMI/MDIO write failed")
end
...
local function smi_read45(smi_bus)
    ...
    local reg = menu.prompt_number("Device register")
    local data = octeon.c.bdk_mdio_45_read(node, smi_bus, phy_addr, dev_addr, reg)
    assert(data ~= -1, "SMI/MDIO read failed")
...
local function smi_write45(smi_bus)
    ...
    local reg = menu.prompt_number("Device register")
    local data = menu.prompt_number("Data")
    assert(octeon.c.bdk_mdio_45_write(node, smi_bus, phy_addr, dev_addr, reg, data) ~= -1, "SMI/MDIO write failed")
end

local function smi_submenu(smi_bus)
    -- Enable the SMI/MDIO bus if it hasn't been enabled yet
    if octeon.csr.SMIX_EN(smi_bus).EN == 0 then
        octeon.csr.SMIX_EN(smi_bus).EN = 1
    end
