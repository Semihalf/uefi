 ...
 */
static inline uint32_t __bdk_usbd_read_csr32(bdk_usbd_state_t *usb, uint64_t address)
{
 ...
 */
static inline void __bdk_usbd_write_csr32(bdk_usbd_state_t *usb, uint64_t address, uint32_t value)
{
 ...
 *               Bytes transferred
 */
static void __bdk_usbd_callback(bdk_usbd_state_t *usb, bdk_usbd_callback_t reason, int endpoint_num, int bytes_transferred)
{
    if (usb->callback[reason])
    {
        if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
            bdk_dprintf("%s: Calling callback reason=%d endpoint=%d bytes=%d func=%p data=%p\n",
    ...
    {
        if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
            bdk_dprintf("%s: No callback for reason=%d endpoint=%d bytes=%d\n",
 ...
 */
static int __bdk_usbd_device_reset_complete(bdk_usbd_state_t *usb)
{
    bdk_usbcx_ghwcfg2_t usbcx_ghwcfg2;
    bdk_usbcx_ghwcfg3_t usbcx_ghwcfg3;
    bdk_usbcx_doepmsk_t usbcx_doepmsk;
    bdk_usbcx_diepmsk_t usbcx_diepmsk;
    bdk_usbcx_daintmsk_t usbc_daintmsk;
    bdk_usbcx_gnptxfsiz_t gnptxfsiz;
    int fifo_space;
    int i;

    if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: Processing reset\n", __FUNCTION__);

    usbcx_ghwcfg2.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GHWCFG2(usb->index));
    usbcx_ghwcfg3.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GHWCFG3(usb->index));
    ...
    {
        bdk_usbcx_dptxfsizx_t siz;
        siz.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DPTXFSIZX(i, usb->index));
        fifo_space -= siz.s.dptxfsize;
        siz.s.dptxfstaddr = fifo_space;
        __bdk_usbd_write_csr32(usb, BDK_USBCX_DPTXFSIZX(i, usb->index), siz.u32);
    }

    /* Assign half the leftover space to the non periodic tx fifo */
    gnptxfsiz.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GNPTXFSIZ(usb->index));
    gnptxfsiz.s.nptxfdep = fifo_space / 2;
    fifo_space -= gnptxfsiz.s.nptxfdep;
    gnptxfsiz.s.nptxfstaddr = fifo_space;
    __bdk_usbd_write_csr32(usb, BDK_USBCX_GNPTXFSIZ(usb->index), gnptxfsiz.u32);

    /* Assign the remain space to the RX fifo */
    __bdk_usbd_write_csr32(usb, BDK_USBCX_GRXFSIZ(usb->index), fifo_space);
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPMSK(usb->index), usbcx_doepmsk.u32);
    usbcx_diepmsk.u32 = 0;
    usbcx_diepmsk.s.epdisbldmsk = 1;
    usbcx_diepmsk.s.xfercomplmsk = 1;
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DIEPMSK(usb->index), usbcx_diepmsk.u32);

    usbc_daintmsk.u32 = 0;
    usbc_daintmsk.s.inepmsk = -1;
    usbc_daintmsk.s.outepmsk = -1;
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DAINTMSK(usb->index), usbc_daintmsk.u32);

    /* Set all endpoints to NAK */
    for (i=0; i<usbcx_ghwcfg2.s.numdeveps+1; i++)
    {
        bdk_usbcx_doepctlx_t usbc_doepctl;
        usbc_doepctl.u32 = 0;
        usbc_doepctl.s.snak = 1;
        usbc_doepctl.s.usbactep = 1;
        usbc_doepctl.s.mps = (i==0) ? 0 : 64;
        __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPCTLX(i, usb->index), usbc_doepctl.u32);
    }
 ...
 */
int bdk_usbd_initialize(bdk_usbd_state_t *usb,
                                      int usb_port_number,
                                      bdk_usbd_initialize_flags_t flags)
{
    ...
    if (bdk_unlikely(flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: Called\n", __FUNCTION__);
    ...
    if ((usb->init_flags & (BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_XI |
                  BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_GND)) == 0)
    {
        if (__bdk_helper_board_usb_get_clock_type() == USB_CLOCK_TYPE_CRYSTAL_12)
            usb->init_flags |= BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_XI;  /* Only 12 MHZ crystals are supported */
        else
            usb->init_flags |= BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_GND;
    }

    if (usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_GND)
    {
        /* Check for auto ref clock frequency */
        if (!(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_CLOCK_MHZ_MASK))
            switch (__bdk_helper_board_usb_get_clock_type())
            {
                case USB_CLOCK_TYPE_REF_12:
                    usb->init_flags |= BDK_USBD_INITIALIZE_FLAGS_CLOCK_12MHZ;
                    break;
                case USB_CLOCK_TYPE_REF_24:
                    usb->init_flags |= BDK_USBD_INITIALIZE_FLAGS_CLOCK_24MHZ;
                    break;
                case USB_CLOCK_TYPE_REF_48:
                default:
                    usb->init_flags |= BDK_USBD_INITIALIZE_FLAGS_CLOCK_48MHZ;
                    break;
            }
    }
    ...
    usbn_clk_ctl.u64 = BDK_CSR_READ(BDK_USBNX_CLK_CTL(usb->index));
    ...
    if (usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_CLOCK_XO_GND)
    {
        ...
        switch (usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_CLOCK_MHZ_MASK)
        {
            case BDK_USBD_INITIALIZE_FLAGS_CLOCK_12MHZ:
                usbn_clk_ctl.s.p_c_sel = 0;
                break;
            case BDK_USBD_INITIALIZE_FLAGS_CLOCK_24MHZ:
                usbn_clk_ctl.s.p_c_sel = 1;
                break;
            case BDK_USBD_INITIALIZE_FLAGS_CLOCK_48MHZ:
    ...
    BDK_CSR_WRITE(BDK_USBNX_CLK_CTL(usb->index), usbn_clk_ctl.u64);
    ...
    usbn_usbp_ctl_status.u64 = BDK_CSR_READ(BDK_USBNX_USBP_CTL_STATUS(usb->index));
    usbn_usbp_ctl_status.s.ate_reset = 1;
    BDK_CSR_WRITE(BDK_USBNX_USBP_CTL_STATUS(usb->index), usbn_usbp_ctl_status.u64);
    /* 6. Wait 10 cycles */
    bdk_wait(10);
        ...
        Global interrupt mask, USBC_GAHBCFG[GLBLINTRMSK] = 1 */
    {
        ...
        __bdk_usbd_write_csr32(usb, BDK_USBCX_GAHBCFG(usb->index), usbcx_gahbcfg.u32);
    }
        ...
        PHY low-power clock select, USBC_GUSBCFG[PHYLPWRCLKSEL] = 0 */
    {
        ...
        usbcx_gusbcfg.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GUSBCFG(usb->index));
        usbcx_gusbcfg.s.toutcal = 0;
        usbcx_gusbcfg.s.ddrsel = 0;
        usbcx_gusbcfg.s.usbtrdtim = 0x5;
        usbcx_gusbcfg.s.phylpwrclksel = 0;
        __bdk_usbd_write_csr32(usb, BDK_USBCX_GUSBCFG(usb->index), usbcx_gusbcfg.u32);
    }
    ...
    {
        bdk_usbcx_dcfg_t usbcx_dcfg;
        usbcx_dcfg.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DCFG(usb->index));
        usbcx_dcfg.s.devspd = 0;
        usbcx_dcfg.s.nzstsouthshk = 0;
        usbcx_dcfg.s.perfrint = 1;
        __bdk_usbd_write_csr32(usb, BDK_USBCX_DCFG(usb->index), usbcx_dcfg.u32);
    }

    /* Program the USBC0/1_GINTMSK register */
    {
        bdk_usbcx_gintmsk_t usbcx_gintmsk;
        usbcx_gintmsk.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GINTMSK(usb->index));
        ...
        __bdk_usbd_write_csr32(usb, BDK_USBCX_GINTMSK(usb->index), usbcx_gintmsk.u32);
    }

    bdk_usbd_disable(usb);
    return 0;
}
...
{
    bdk_usbcx_dctl_t usbcx_dctl;
    usbcx_dctl.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DCTL(usb->index));
    usbcx_dctl.s.cgoutnak = 1;
    usbcx_dctl.s.sftdiscon = 0;
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DCTL(usb->index), usbcx_dctl.u32);
    return 0;
}
...
{
    if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: Register reason=%d func=%p data=%p\n",
 ...
 */
static int __bdk_usbd_poll_in_endpoint(bdk_usbd_state_t *usb, int endpoint_num)
{
    bdk_usbcx_diepintx_t usbc_diepint;

    if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: endpoint=%d\n", __FUNCTION__, endpoint_num);

    usbc_diepint.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DIEPINTX(endpoint_num, usb->index));
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DIEPINTX(endpoint_num, usb->index), usbc_diepint.u32);
    ...
    {
        bdk_usbcx_dieptsizx_t usbc_dieptsiz;
        ...
        usbc_dieptsiz.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DIEPTSIZX(endpoint_num, usb->index));
        bytes_transferred = usb->endpoint[endpoint_num].buffer_length - usbc_dieptsiz.s.xfersize;
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_IN_COMPLETE, endpoint_num, bytes_transferred);
    }
    return 0;
}
 ...
 * @return Zero on success
 */
static int __bdk_usbd_poll_out_endpoint(bdk_usbd_state_t *usb, int endpoint_num)
{
    bdk_usbcx_doepintx_t usbc_doepint;

    if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: endpoint=%d\n", __FUNCTION__, endpoint_num);

    usbc_doepint.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DOEPINTX(endpoint_num, usb->index));
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPINTX(endpoint_num, usb->index), usbc_doepint.u32);
    ...
    if (usbc_doepint.s.setup)
    {
        ...
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_DEVICE_SETUP, endpoint_num, 0);
    }
    ...
    {
        bdk_usbcx_doeptsizx_t usbc_doeptsiz;
        int bytes_transferred;
        /* Transfer Completed Interrupt (XferCompl)
            Indicates that the programmed transfer is complete on the AHB
            as well as on the USB, for this endpoint. */
        usbc_doeptsiz.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DOEPTSIZX(endpoint_num, usb->index));
        bytes_transferred = usb->endpoint[endpoint_num].buffer_length - usbc_doeptsiz.s.xfersize;
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_OUT_COMPLETE, endpoint_num, bytes_transferred);
    }
...
int bdk_usbd_poll(bdk_usbd_state_t *usb)
{
    ...
    usbc_gintsts.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_GINTSTS(usb->index));
    usbc_gintsts.u32 &= __bdk_usbd_read_csr32(usb, BDK_USBCX_GINTMSK(usb->index));

    /* Clear the interrupts now that we know about them */
    __bdk_usbd_write_csr32(usb, BDK_USBCX_GINTSTS(usb->index), usbc_gintsts.u32);

    if (usbc_gintsts.s.usbsusp)
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_SUSPEND, 0, 0);

    if (usbc_gintsts.s.enumdone)
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_ENUM_COMPLETE, 0, 0);
    ...
    if (usbc_gintsts.s.usbrst)
    {
        /* USB Reset (USBRst)
            ...
            detected on the USB. */
        __bdk_usbd_device_reset_complete(usb);
        __bdk_usbd_callback(usb, BDK_USBD_CALLBACK_RESET, 0, 0);
    }

    if (usbc_gintsts.s.oepint || usbc_gintsts.s.iepint)
    {
        bdk_usbcx_daint_t usbc_daint;
        usbc_daint.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DAINT(usb->index));
        if (usbc_daint.s.inepint)
        {
            ...
            {
                int endpoint;
                BDK_CLZ(endpoint, active_endpoints);
                endpoint = 31 - endpoint;
                __bdk_usbd_poll_in_endpoint(usb, endpoint);
        ...
        if (usbc_daint.s.outepint)
        {
            ...
            while (active_endpoints)
            {
                ...
                BDK_CLZ(endpoint, active_endpoints);
                endpoint = 31 - endpoint;
                __bdk_usbd_poll_out_endpoint(usb, endpoint);
                active_endpoints ^= 1<<endpoint;
            }
        }
    }
...
{
    bdk_usbcx_dcfg_t usbc_dcfg;
    usbc_dcfg.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DCFG(usb->index));
...
void bdk_usbd_set_address(bdk_usbd_state_t *usb, int address)
{
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DCFG(usb->index), usbc_dcfg.u32);
}
...
{
    bdk_usbcx_dsts_t usbcx_dsts;
    usbcx_dsts.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DSTS(usb->index));
    ...
    int max_packet_size, uint64_t buffer, int buffer_length)
{
    ...
    if (bdk_unlikely(usb->init_flags & BDK_USBD_INITIALIZE_FLAGS_DEBUG))
        bdk_dprintf("%s: endpoint=%d buffer=0x%llx length=%d\n",
            __FUNCTION__, endpoint_num, (ULL)buffer, buffer_length);

    usb->endpoint[endpoint_num].buffer_length = buffer_length;

    BDK_SYNCW; /* Flush out pending writes before enable */

    /* Clear any pending interrupts */
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPINTX(endpoint_num, usb->index),
        __bdk_usbd_read_csr32(usb, BDK_USBCX_DOEPINTX(endpoint_num, usb->index)));

    /* Setup the locations the DMA engines use  */
    bdk_write64_uint64(BDK_USBNX_DMA0_INB_CHN0(usb->index) + endpoint_num*8, buffer);
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPTSIZX(endpoint_num, usb->index), usbc_doeptsiz.u32);
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DOEPCTLX(endpoint_num, usb->index), usbc_doepctl.u32);
...
int bdk_usbd_out_endpoint_disable(bdk_usbd_state_t *usb, int endpoint_num)
{
    ...
    usbc_doepctl.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DOEPCTLX(endpoint_num, usb->index));
...
int bdk_usbd_in_endpoint_enable(bdk_usbd_state_t *usb,
    ...
    int max_packet_size, uint64_t buffer, int buffer_length)
{
    bdk_usbcx_diepctlx_t usbc_diepctl;
    bdk_usbcx_dieptsizx_t usbc_dieptsiz;
    ...
    /* Clear any pending interrupts */
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DIEPINTX(endpoint_num, usb->index),
        __bdk_usbd_read_csr32(usb, BDK_USBCX_DIEPINTX(endpoint_num, usb->index)));

    usbc_dieptsiz.u32 = 0;
    usbc_dieptsiz.s.mc = 1;
    if (buffer)
    {
        bdk_write64_uint64(BDK_USBNX_DMA0_OUTB_CHN0(usb->index) + endpoint_num*8, buffer);
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DIEPTSIZX(endpoint_num, usb->index), usbc_dieptsiz.u32);
    ...
    __bdk_usbd_write_csr32(usb, BDK_USBCX_DIEPCTLX(endpoint_num, usb->index), usbc_diepctl.u32);
...
int bdk_usbd_in_endpoint_disable(bdk_usbd_state_t *usb, int endpoint_num)
{
    ...
    usbc_diepctl.u32 = __bdk_usbd_read_csr32(usb, BDK_USBCX_DIEPCTLX(endpoint_num, usb->index));
