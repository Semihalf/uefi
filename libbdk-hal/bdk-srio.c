...
typedef struct
{
    bdk_srio_initialize_flags_t flags;
...
static int __bdk_srio_alloc_s2m(int srio_port, bdk_sriox_s2m_typex_t s2m)
{
    ...
    for (s2m_index=0; s2m_index<16; s2m_index++)
    {
        /* Increment ref count by 2 since we count read and write
            ...
            future */
        int ref_count = bdk_atomic_fetch_and_add32(&__bdk_srio_state[srio_port].s2m_ref_count[s2m_index], 2);
        if (ref_count == 0)
        {
        ...
        else
        {
            ...
            if (BDK_CSR_READ(BDK_SRIOX_S2M_TYPEX(s2m_index, srio_port)) == s2m.u64)
                return s2m_index;
            else
...
/**
 * @INTERNAL
 * Free a handle allocated by __bdk_srio_alloc_s2m
 *
 * @param srio_port SRIO port
 * @param index     Index to free
 */
static void __bdk_srio_free_s2m(int srio_port, int index)
{
    /* Read to force pending transactions to complete */
    BDK_CSR_READ(BDK_SRIOX_S2M_TYPEX(index, srio_port));
    bdk_atomic_add32(&__bdk_srio_state[srio_port].s2m_ref_count[index], -2);
}
...
static int __bdk_srio_alloc_subid(bdk_sli_mem_access_subidx_t subid)
{
    ...
    {
        int ref_count = bdk_atomic_fetch_and_add32(&__bdk_srio_state[0].subidx_ref_count[mem_index-12], 1);
        if (ref_count == 0)
        {
            /* Unused location. Write our value */
        ...
        {
            /* In use, see if we can use it */
            if (BDK_CSR_READ(BDK_SLI_MEM_ACCESS_SUBIDX(mem_index)) == subid.u64)
                return mem_index;
            else
...
/**
 * @INTERNAL
 * Free a handle allocated by __bdk_srio_alloc_subid
 *
 * @param index  Index to free
 */
static void __bdk_srio_free_subid(int index)
{
    /* Read to force pending transactions to complete */
    BDK_CSR_READ(BDK_SLI_MEM_ACCESS_SUBIDX(index));
    bdk_atomic_add32(&__bdk_srio_state[0].subidx_ref_count[index-12], -1);
}
...
static int __bdk_srio_local_read32(int srio_port, uint32_t offset, uint32_t *result)
{
    ...
    BDK_CSR_WRITE(BDK_SRIOX_MAINT_OP(srio_port), maint_op.u64);
    ...
    /* Get the read data */
    maint_rd_data.u64 = BDK_CSR_READ(BDK_SRIOX_MAINT_RD_DATA(srio_port));
 ...
 * @return Zero on success, negative on failure.
 */
int bdk_srio_config_write32(int srio_port, int srcid_index, int destid,
                             int is16bit, uint8_t hopcount, uint32_t offset,
                             uint32_t data)
{
    if (destid == -1)
    {
        if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
            bdk_dprintf("SRIO%d: Local write[0x%06x] => 0x%08x\n", srio_port, (unsigned int)offset, (unsigned int)data);

        return __bdk_srio_local_write32(srio_port, offset, data);
    }
    else
    {
        if (OCTEON_IS_MODEL(OCTEON_CN63XX_PASS1_X))
 ...
 * @return Zero on success, negative on failure.
 */
int bdk_srio_send_doorbell(int srio_port, int srcid_index, int destid, int is16bit, int priority, uint16_t data)
{
    bdk_sriox_tx_bell_t tx_bell;
    tx_bell.u64 = 0;
    tx_bell.s.data = data;
    ...
    if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
        bdk_dprintf("SRIO%d: Send doorbell destid=0x%x, priority=%d, data=0x%x\n", srio_port, destid, priority, 0xffff & data);
    ...
    BDK_CSR_WRITE(BDK_SRIOX_TX_BELL(srio_port), tx_bell.u64);
    return 0;
}
 ...
 */
bdk_srio_doorbell_status_t bdk_srio_send_doorbell_status(int srio_port)
{
    bdk_sriox_tx_bell_t tx_bell;
    bdk_sriox_tx_bell_info_t tx_bell_info;
    bdk_sriox_int_reg_t int_reg;
    bdk_sriox_int_reg_t int_reg_clear;

    /* Return busy if the doorbell is still processing */
    tx_bell.u64 = BDK_CSR_READ(BDK_SRIOX_TX_BELL(srio_port));
    if (tx_bell.s.pending)
        return BDK_SRIO_DOORBELL_BUSY;

    /* Read and clear the TX doorbell interrupts */
    int_reg.u64 = BDK_CSR_READ(BDK_SRIOX_INT_REG(srio_port));
    int_reg_clear.u64 = 0;
    int_reg_clear.s.bell_err = int_reg.s.bell_err;
    int_reg_clear.s.txbell = int_reg.s.txbell;
    BDK_CSR_WRITE(BDK_SRIOX_INT_REG(srio_port), int_reg_clear.u64);

    /* Check for errors */
    if (int_reg.s.bell_err)
    {
        if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
            bdk_dprintf("SRIO%d: Send doorbell failed\n", srio_port);
        tx_bell_info.u64 = BDK_CSR_READ(BDK_SRIOX_TX_BELL_INFO(srio_port));
        if (tx_bell_info.s.timeout || tx_bell_info.s.error)
            return BDK_SRIO_DOORBELL_ERROR;
        if (tx_bell_info.s.retry)
            return BDK_SRIO_DOORBELL_RETRY;
    }

    /* Check if we're done */
    if (int_reg.s.txbell)
        return BDK_SRIO_DOORBELL_DONE;

    /* No doorbell found */
    return BDK_SRIO_DOORBELL_NONE;
}
 ...
 */
bdk_srio_doorbell_status_t bdk_srio_receive_doorbell(int srio_port,
        int *destid_index, uint32_t *sequence_num, int *srcid, int *priority,
        int *is16bit, uint16_t *data)
{
    bdk_sriox_rx_bell_seq_t rx_bell_seq;
    bdk_sriox_rx_bell_t rx_bell;

    /* Check if there are any pending doorbells */
    rx_bell_seq.u64 = BDK_CSR_READ(BDK_SRIOX_RX_BELL_SEQ(srio_port));
    if (!rx_bell_seq.s.count)
        return BDK_SRIO_DOORBELL_NONE;

    /* Read the doorbell and write our return parameters */
    rx_bell.u64 = BDK_CSR_READ(BDK_SRIOX_RX_BELL(srio_port));
    ...
    *destid_index = rx_bell.s.dest_id;
    ...
    if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
        bdk_dprintf("SRIO%d: Receive doorbell sequence=0x%x, srcid=0x%x, priority=%d, data=0x%x\n",
 ...
 */
int bdk_srio_receive_spf(int srio_port, void *buffer, int buffer_length)
{
    uint32_t *ptr = (uint32_t *)buffer;
    bdk_sriomaintx_ir_sp_rx_stat_t sriomaintx_ir_sp_rx_stat;

    /* Read the SFP status */
    if (__bdk_srio_local_read32(srio_port, BDK_SRIOMAINTX_IR_SP_RX_STAT(srio_port), &sriomaintx_ir_sp_rx_stat.u32))
        return -1;
    ...
    if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
        bdk_dprintf("SRIO%d: Soft packet FIFO received %d bytes", srio_port, sriomaintx_ir_sp_rx_stat.s.octets);
    ...
    {
        __bdk_srio_local_read32(srio_port, BDK_SRIOMAINTX_IR_SP_RX_DATA(srio_port), ptr);
        if (__bdk_srio_state[srio_port].flags & BDK_SRIO_INITIALIZE_DEBUG)
            bdk_dprintf(" %08x", (unsigned int)*ptr);
        ptr++;
        buffer_length-=4;
    }
 ...
 *         or zero on failure.
 */
uint64_t bdk_srio_physical_map(int srio_port, bdk_srio_write_mode_t write_op,
        int write_priority, bdk_srio_read_mode_t read_op, int read_priority,
        int srcid_index, int destid, int is16bit, uint64_t base, uint64_t size)
{
    bdk_sriox_s2m_typex_t needed_s2m_type;
    bdk_sli_mem_access_subidx_t needed_subid;
    int s2m_index;
    int subdid;
    bdk_sli_address_t sli_address;
    ...
    s2m_index = __bdk_srio_alloc_s2m(srio_port, needed_s2m_type);
    if (s2m_index == -1)
        return 0;
    ...
    subdid = __bdk_srio_alloc_subid(needed_subid);
    if (subdid == -1)
    {
        /* Free the s2m_index as we aren't using it */
        __bdk_srio_free_s2m(srio_port, s2m_index);
        return 0;
    }
...
/**
 * Unmap a physical address window created by bdk_srio_phys_map().
 *
 * @param physical_address
 *               Physical address returned by bdk_srio_phys_map().
...
{
    bdk_sli_mem_access_subidx_t subid;
    ...
    subid.u64 = BDK_CSR_READ(BDK_SLI_MEM_ACCESS_SUBIDX(mem_index));
