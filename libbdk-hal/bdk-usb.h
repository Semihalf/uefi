 ...
 * @file
 *
 * "bdk-usb.h" defines a set of low level USB functions to help
 ...
 *
 * -# Call bdk_usb_get_num_ports() to get the number of
 *  supported ports.
 * -# Call bdk_usb_initialize() for each Octeon USB port.
 * -# Enable the port using bdk_usb_enable().
 * -# Either periodically, or in an interrupt handler, call
 *  bdk_usb_poll() to service USB events.
 * -# Manage pipes using bdk_usb_open_pipe() and
 *  bdk_usb_close_pipe().
 * -# Manage transfers using bdk_usb_submit_*() and
 *  bdk_usb_cancel*().
 * -# Shutdown USB on unload using bdk_usb_shutdown().
 *
 * To monitor USB status changes, the device driver must use
 * bdk_usb_register_callback() to register for events that it
 ...
 * interrupts before every call to bdk_usb*() and enable them
 ...
 *
 * void port_callback(bdk_usb_state_t *usb,
 *                    bdk_usb_callback_t reason,
 *                    bdk_usb_complete_t status,
 *                    int pipe_handle,
 *                    int submit_handle,
 *                    int bytes_transferred,
 *                    void *user_data);
 * - @b usb is the bdk_usb_state_t for the port.
 * - @b reason will always be
 *   BDK_USB_CALLBACK_PORT_CHANGED.
 * - @b status will always be BDK_USB_COMPLETE_SUCCESS.
 ...
 * uses bdk_usb_get_status() to determine the root port status.
 *
 * <h2>Complete callback</h2>
 *
 * The completion callback prototype needs to look as follows:
 *
 * void complete_callback(bdk_usb_state_t *usb,
 *                        bdk_usb_callback_t reason,
 *                        bdk_usb_complete_t status,
 *                        int pipe_handle,
 *                        int submit_handle,
 ...
 * - @b status will be one of the bdk_usb_complete_t
 *   enumerations.
 ...
 *
 * When a transaction is cancelled using bdk_usb_cancel*(), the
 * actual length of time until the complete callback is called
 * can vary greatly. It may be called before bdk_usb_cancel*()
 ...
 * {
 *     bdk_usb_state_t *usb = (bdk_usb_state_t *)hcd->hcd_priv;
 *     bdk_usb_port_status_t usb_port_status;
 *     int port_status;
 ...
 *                     bdk_usb_set_status(usb, bdk_usb_get_status(usb));
 *                     break;
 ...
 *             }
 *
 *             usb_port_status = bdk_usb_get_status(usb);
 *             port_status = 0;
 *
 ...
 *             }
 *
 *             if (usb_port_status.port_speed == BDK_USB_SPEED_HIGH)
 *             {
 *                 port_status |= (1 << USB_PORT_FEAT_HIGHSPEED);
 *                 DEBUG_ROOT_HUB(" HIGHSPEED");
 *             }
 *             else if (usb_port_status.port_speed == BDK_USB_SPEED_LOW)
 *             {
 ...
 * <h2>Interrupts</h2>
 *
 * If you plan on using usb interrupts, bdk_usb_poll() must be
 ...
 * handler and call bdk_usb_poll(). Octeon's usb port 0 causes
 ...
 *
 * If you aren't using interrupts, simple call bdk_usb_poll()
...
typedef enum
{
    BDK_USB_SUCCESS = 0,           /**< There were no errors */
    BDK_USB_INVALID_PARAM = -1,    /**< A parameter to the function was invalid */
    BDK_USB_NO_MEMORY = -2,        /**< Insufficient resources were available for the request */
    BDK_USB_BUSY = -3,             /**< The resource is busy and cannot service the request */
    BDK_USB_TIMEOUT = -4,          /**< Waiting for an action timed out */
    BDK_USB_INCORRECT_MODE = -5,   /**< The function call doesn't work in the current USB
...
typedef enum
{
    BDK_USB_SPEED_HIGH = 0,        /**< Device is operation at 480Mbps */
    BDK_USB_SPEED_FULL = 1,        /**< Device is operation at 12Mbps */
    BDK_USB_SPEED_LOW = 2,         /**< Device is operation at 1.5Mbps */
} bdk_usb_speed_t;
...
typedef enum
{
    BDK_USB_TRANSFER_CONTROL = 0,      /**< USB transfer type control for hub and status transfers */
    BDK_USB_TRANSFER_ISOCHRONOUS = 1,  /**< USB transfer type isochronous for low priority periodic transfers */
    BDK_USB_TRANSFER_BULK = 2,         /**< USB transfer type bulk for large low priority transfers */
    BDK_USB_TRANSFER_INTERRUPT = 3,    /**< USB transfer type interrupt for high priority periodic transfers */
} bdk_usb_transfer_t;
...
typedef enum
{
    BDK_USB_DIRECTION_OUT,         /**< Data is transferring from Octeon to the device/host */
    BDK_USB_DIRECTION_IN,          /**< Data is transferring from the device/host to Octeon */
} bdk_usb_direction_t;
...
typedef enum
{
    BDK_USB_COMPLETE_SUCCESS,      /**< The transaction / operation finished without any errors */
    BDK_USB_COMPLETE_SHORT,        /**< FIXME: This is currently not implemented */
    BDK_USB_COMPLETE_CANCEL,       /**< The transaction was canceled while in flight by a user call to bdk_usb_cancel* */
    BDK_USB_COMPLETE_ERROR,        /**< The transaction aborted with an unexpected error status */
    BDK_USB_COMPLETE_STALL,        /**< The transaction received a USB STALL response from the device */
    BDK_USB_COMPLETE_XACTERR,      /**< The transaction failed with an error from the device even after a number of retries */
    BDK_USB_COMPLETE_DATATGLERR,   /**< The transaction failed with a data toggle error even after a number of retries */
    BDK_USB_COMPLETE_BABBLEERR,    /**< The transaction failed with a babble error */
    BDK_USB_COMPLETE_FRAMEERR,     /**< The transaction failed with a frame error even after a number of retries */
} bdk_usb_complete_t;
...
typedef struct
{
    ...
    bdk_usb_speed_t port_speed : 2; /**< Current port speed */
...
typedef struct
{
    ...
    bdk_usb_complete_t status;     /**< This is the status of this individual packet transfer */
} bdk_usb_iso_packet_t;
...
typedef enum
{
    ...
    BDK_USB_CALLBACK_PORT_CHANGED, /**< The status of the port changed. For example, someone may have
                                        plugged a device in. The status parameter contains
                                        BDK_USB_COMPLETE_SUCCESS. Use bdk_usb_get_status() to get
                                        the new port status. */
    __BDK_USB_CALLBACK_END         /**< Do not use. Used internally for array bounds */
} bdk_usb_callback_t;
 ...
 *        bdk_usb_initialize().
 *      - reason = The bdk_usb_callback_t used to register
 *        the callback.
 *      - status = The bdk_usb_complete_t representing the
 ...
 *        function bdk_usb_submit() or
 *        bdk_usb_register_callback() */
typedef void (*bdk_usb_callback_func_t)(bdk_usb_state_t *state,
                                         bdk_usb_callback_t reason,
                                         bdk_usb_complete_t status,
...
typedef enum
{
    BDK_USB_INITIALIZE_FLAGS_CLOCK_XO_XI = 1<<0,       /**< The USB port uses a 12MHz crystal as clock source
                                                            at USB_XO and USB_XI. */
    BDK_USB_INITIALIZE_FLAGS_CLOCK_XO_GND = 1<<1,      /**< The USB port uses 12/24/48MHz 2.5V board clock
                                                            source at USB_XO. USB_XI should be tied to GND.*/
    BDK_USB_INITIALIZE_FLAGS_CLOCK_AUTO = 0,           /**< Automatically determine clock type based on function
                                                             in cvmx-helper-board.c. */
    BDK_USB_INITIALIZE_FLAGS_CLOCK_MHZ_MASK  = 3<<3,       /**< Mask for clock speed field */
    BDK_USB_INITIALIZE_FLAGS_CLOCK_12MHZ = 1<<3,       /**< Speed of reference clock or crystal */
    BDK_USB_INITIALIZE_FLAGS_CLOCK_24MHZ = 2<<3,       /**< Speed of reference clock */
    BDK_USB_INITIALIZE_FLAGS_CLOCK_48MHZ = 3<<3,       /**< Speed of reference clock */
    /* Bits 3-4 used to encode the clock frequency */
    BDK_USB_INITIALIZE_FLAGS_NO_DMA = 1<<5,            /**< Disable DMA and used polled IO for data transfer use for the USB  */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_TRANSFERS = 1<<16,  /**< Enable extra console output for debugging USB transfers */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_CALLBACKS = 1<<17,  /**< Enable extra console output for debugging USB callbacks */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_INFO = 1<<18,       /**< Enable extra console output for USB informational data */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_CALLS = 1<<19,      /**< Enable extra console output for every function call */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_CSRS = 1<<20,       /**< Enable extra console output for every CSR access */
    BDK_USB_INITIALIZE_FLAGS_DEBUG_ALL = ((BDK_USB_INITIALIZE_FLAGS_DEBUG_CSRS<<1)-1) - (BDK_USB_INITIALIZE_FLAGS_DEBUG_TRANSFERS-1),
} bdk_usb_initialize_flags_t;
...
typedef enum
{
    BDK_USB_PIPE_FLAGS_DEBUG_TRANSFERS = 1<<15,/**< Used to display BDK_USB_INITIALIZE_FLAGS_DEBUG_TRANSFERS for a specific pipe only */
    __BDK_USB_PIPE_FLAGS_OPEN = 1<<16,         /**< Used internally to determine if a pipe is open. Do not use */
    __BDK_USB_PIPE_FLAGS_SCHEDULED = 1<<17,    /**< Used internally to determine if a pipe is actively using hardware. Do not use */
    __BDK_USB_PIPE_FLAGS_NEED_PING = 1<<18,    /**< Used internally to determine if a high speed pipe is in the ping state. Do not use */
 ...
 */
extern int bdk_usb_get_num_ports(void);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_initialize(bdk_usb_state_t *state,
                                             int usb_port_number,
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_shutdown(bdk_usb_state_t *state);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_enable(bdk_usb_state_t *state);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_disable(bdk_usb_state_t *state);
 ...
 */
extern bdk_usb_port_status_t bdk_usb_get_status(bdk_usb_state_t *state);
 ...
 */
extern void bdk_usb_set_status(bdk_usb_state_t *state, bdk_usb_port_status_t port_status);
 ...
 */
extern int bdk_usb_open_pipe(bdk_usb_state_t *state,
                              bdk_usb_pipe_flags_t flags,
 ...
 *         bdk_usb_status_t.
 */
extern int bdk_usb_submit_bulk(bdk_usb_state_t *state, int pipe_handle,
 ...
 *         bdk_usb_status_t.
 */
extern int bdk_usb_submit_interrupt(bdk_usb_state_t *state, int pipe_handle,
                                     uint64_t buffer, int buffer_length,
                                     bdk_usb_callback_func_t callback,
                                     void *user_data);
 ...
 *         bdk_usb_status_t.
 */
extern int bdk_usb_submit_control(bdk_usb_state_t *state, int pipe_handle,
...
/**
 * Flags to pass the bdk_usb_submit_isochronous() function.
 */
typedef enum
{
    BDK_USB_ISOCHRONOUS_FLAGS_ALLOW_SHORT = 1<<0,  /**< Do not return an error if a transfer is less than the maximum packet size of the device */
    BDK_USB_ISOCHRONOUS_FLAGS_ASAP = 1<<1,         /**< Schedule the transaction as soon as possible */
} bdk_usb_isochronous_flags_t;
 ...
 *         bdk_usb_status_t.
 */
extern int bdk_usb_submit_isochronous(bdk_usb_state_t *state, int pipe_handle,
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_cancel(bdk_usb_state_t *state,
                                         int pipe_handle, int submit_handle);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_cancel_all(bdk_usb_state_t *state,
                                             int pipe_handle);
 ...
 */
extern bdk_usb_status_t bdk_usb_close_pipe(bdk_usb_state_t *state,
                                             int pipe_handle);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_register_callback(bdk_usb_state_t *state,
                                                    bdk_usb_callback_t reason,
                                                    bdk_usb_callback_func_t callback,
                                                    void *user_data);
 ...
 */
extern int bdk_usb_get_frame_number(bdk_usb_state_t *state);
 ...
 *         bdk_usb_status_t.
 */
extern bdk_usb_status_t bdk_usb_poll(bdk_usb_state_t *state);
